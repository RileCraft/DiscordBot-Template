...
export async function run(client, message, args) {
    const deleteOutputComponent = new ActionRowBuilder()
        .addComponents(
            new ButtonBuilder()
    ...
    if (!code) return message.channel.send({ content: "Please specify something to Evaluate" });
    try {
        ...
        if (originalCode.includes("--silent")) return;
            ...
            content: `\`\`\`js\n${code}\n\`\`\``,
            components: [deleteOutputComponent],
            allowedMentions: {
                repliedUser: false
            }
        });
