...
else message.channel.send({ embeds: [embedLoader.cooldown(message.author, data.coolTime[0])] })
 }
			else if (!validator.ownerOnly(command, message, false)) {
if (command.returnOwnerOnlyError === false || command.returnNoErrors) return; 
else message.channel.send({ embeds: [embedLoader.ownerOnly(message.author)] })
 }
...
else message.channel.send({ embeds: [embedLoader.userPermissions(message.author, data.missingUserPermissions)] })
  }
...
else message.channel.send({ embeds: [embedLoader.clientPermissions(message.author, data.missingClientPermissions)] })
 }
...
else message.channel.send({ embeds: [embedLoader.anyUserPermissions(message.author, data.missingAnyUserPermissions)] })
  }
...
else message.channel.send({ embeds: [embedLoader.anyClientPermissions(message.author, data.missingAnyClientPermissions)] })
 }
						else if (!validator.onlyUsers(command, message, false)) {
if (command.returnOnlyUsersError === false || command.returnNoErrors) return;
else message.channel.send({ embeds: [embedLoader.onlyUsers(message.author, data.missingUsers)] })
  }
  else if (!validator.onlyRoles(command, message, false)) {
if (command.returnOnlyRolesError === false || command.returnNoErrors) return;
else message.channel.send({ embeds: [embedLoader.onlyRoles(message.author, data.missingRoles)] })
  }
...
else message.channel.send({ embeds: [embedLoader.onlyChannels(message.author, data.missingChannels)] })
  }
  else if (!validator.onlyGuilds(command, message, false)) {
if (command.returnOnlyGuildsError === false || command.returnNoErrors) return;
else message.channel.send({ embeds: [embedLoader.onlyGuilds(message.author, data.missingGuilds)] })
  }
